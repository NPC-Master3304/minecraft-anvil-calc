...
import { Enchantment, ItemData, ItemSpecification, Preset, Settings, StepData } from "./models";
...
class App extends React.Component<Record<string, never>, AppState> {
  ...
  getAddOptions() {
    ...
    const filtered_items = (items as Array<ItemSpecification>).filter((item) => {
      return (
        (this.state.settings.java_edition || !item.java_only) &&
        ...
        <Container fluid>
          ...
          <Row>
            ...
            <Col xs="6">
              <h3>Results</h3>
              <p>{results.status}</p>
              {results.steps.some((step) => step.tooExpensive) && (
                <Alert variant="danger">
                  At least one step costs {MAX_ANVIL_COST} levels or more, so the
                  anvil will show &quot;Too Expensive!&quot; in survival. Reduce the prior
                  work penalties or split the combination.
                </Alert>
              )}
